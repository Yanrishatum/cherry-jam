...
class DebugDisplay
{
  ...
  static var infos:Array<Text>;
  static var infoPool:Array<Text>;
  ...
  public static function init():Void
  {
    ...
    base = new Layers();
    flow = new h2d.Flow(base);
    graphics = new Graphics(base);
    flow.layout = Vertical;
    flow.verticalSpacing = 2;
    infos = new Array();
    infoPool = new Array();
    base.add(new SyncSnatcher(), 100);
  }
  
  public static function attach(s2d:Scene)
  {
    s2d.addChild(base);
  }
  
  static function updateWatch()
  {
    graphics.clear();
  ...
  public static function info(v:String, x:Float = 10, y:Float = 10)
  {
    var txt:Text;
    if (infoPool.length != 0) txt = infoPool.pop();
    else {
      txt = new Text(font, base);
      txt.dropShadow = { dx: 1, dy: 1, color: 0, alpha: 0.5 };
    }
    txt.setPosition(x, y);
    txt.text = v;
    txt.visible = true;
...
private class SyncSnatcher extends Object
{
  override private function drawRec(ctx:RenderContext)
  {
    super.drawRec(ctx);
    @:privateAccess DebugDisplay.updateWatch();
  }
}